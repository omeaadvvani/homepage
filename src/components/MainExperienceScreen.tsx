...
const MainExperienceScreen: React.FC<MainExperienceScreenProps> = ({ 
  ...
  const { preferences, loading: prefsLoading, error: prefsError } = useUserPreferences();
  ...
  // Determine if we have the minimum data needed
  const hasMinimumData = user && (preferences || userProfile);
  const hasError = !!prefsError;
  ...
  return (
    <SafeRenderer
      isLoading={isLoading}
      hasError={hasError}
      dataReady={hasMinimumData}
      errorMessage={prefsError || "Unable to load your preferences. Please try again."}
    >
