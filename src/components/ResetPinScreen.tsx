...
const ResetPinScreen: React.FC<ResetPinScreenProps> = ({ onComplete, onBack }) => {
  ...
  // Determine data readiness - for reset PIN, we just need the form to be ready
  const isDataReady = true; // Reset PIN screen doesn't depend on external data
  const hasError = false; // No critical errors that would prevent rendering
  const isLoading = false; // No initial loading state for reset PIN
  ...
  return (
    <SafeRenderer
      ...
      errorMessage="Unable to load PIN reset form. Please refresh the page."
    >
