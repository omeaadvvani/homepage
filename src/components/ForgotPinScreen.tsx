...
const ForgotPinScreen: React.FC<ForgotPinScreenProps> = ({ onBack, onComplete }) => {
  ...
  // Determine data readiness - for forgot PIN, we just need the form to be ready
  const isDataReady = true; // Forgot PIN screen doesn't depend on external data
  const hasError = false; // No critical errors that would prevent rendering
  const isLoading = false; // No initial loading state for forgot PIN
  ...
  return (
    <SafeRenderer
      ...
      errorMessage="Unable to load forgot PIN form. Please refresh the page."
    >
