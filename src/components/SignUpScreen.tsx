...
const SignUpScreen: React.FC<SignUpScreenProps> = ({ onComplete, onBack }) => {
  ...
  // Determine data readiness - for signup, we just need the form to be ready
  const isDataReady = true; // Signup screen doesn't depend on external data
  const hasError = false; // No critical errors that would prevent rendering
  const isLoading = false; // No initial loading state for signup
  ...
  return (
    <SafeRenderer
      ...
      errorMessage="Unable to load sign up form. Please refresh the page."
    >
                ...
                <div className="flex justify-center gap-2 mb-4">
                  {pin.map((digit, index) => (
                    <input
                      ...
                      disabled={loading}
                      className="w-10 h-10 text-center text-lg font-bold border-2 border-spiritual-200 rounded-spiritual focus:border-spiritual-400 focus:outline-none focus:ring-4 focus:ring-spiritual-200/50 transition-all duration-300 bg-white/70 text-spiritual-900 hover:border-spiritual-300 focus:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
